 ...
 * {@link OutlineViewElementTextProviderOld} and ensure that they are not null. We can avoid checking for thing used by
 * {@link OutlineViewIconProviderOld} because that just uses the modifiers. And the modifiers are always available (because
 * we parse from left to right). This is specific to the outline and not how you would handle it in general.
 * 
 * @author nchen
 * 
 */
class BlockMemberFilterOld extends BlockMember.SwitchBlockWithDefault {

    final private List<Object> children;

    public BlockMemberFilterOld(List<Object> children) {
