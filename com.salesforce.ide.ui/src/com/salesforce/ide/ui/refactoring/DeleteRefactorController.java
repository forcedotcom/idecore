...
public class DeleteRefactorController extends BaseRefactorController {
     ...
     */
    public RefactoringStatus validateDelete(IProgressMonitor monitor) throws OperationCanceledException,
        ...
        for (IResource deleteCandidate : deleteCandidates) {
            ...
            monitorCheck(monitor);
            boolean forceManaged = ContainerDelegate.getInstance().getServiceLocator().getProjectService().isManagedResource(deleteCandidate);
            if (!forceManaged) {
                if (logger.isInfoEnabled()) {
                    logger.info("Delete resource '" + deleteCandidate.getFullPath().toPortableString() + " is not "
                            + Constants.PLUGIN_NAME + " managed; therefore, we don't care!");
                }
                continue;
            }
            ...
            if (deleteCandidate.getType() == IResource.FILE) {
                ...
                if (Constants.SCHEMA_FILENAME.equals(deleteCandidate.getName())
                    ...
                    logger.error(Constants.PLUGIN_NAME + " does not support deleting '" + deleteCandidate.getName()
                            + "'");
                    refactoringStatus.addEntry(createFatalRefactoringStatusEntry(Constants.PLUGIN_NAME
                            + " does not support deleting '" + deleteCandidate.getName() + "'"));
                }
                ...
                if (ContainerDelegate.getInstance().getServiceLocator().getProjectService().isReferencedPackageResource(deleteCandidate)
                        || ContainerDelegate.getInstance().getServiceLocator().getProjectService().isReferencedPackagesFolder(deleteCandidate)) {
                    logger.error(Constants.PLUGIN_NAME + " does not support deleting 'Referenced Package' resource '"
                            + deleteCandidate.getName() + "'");
                    refactoringStatus.addEntry(createFatalRefactoringStatusEntry(Constants.PLUGIN_NAME
                            + " does not support deleting 'Referenced Package' resource '" + deleteCandidate.getName()
                            + "'"));
                }

                // cannot delete src folder
                if (ContainerDelegate.getInstance().getServiceLocator().getProjectService().isSourceFolder(deleteCandidate)) {
                    logger.error(Constants.PLUGIN_NAME + " does not support deleting package source folder '"
                            + deleteCandidate.getName() + "'");
                    refactoringStatus.addEntry(createFatalRefactoringStatusEntry(Constants.PLUGIN_NAME
                            + " does not support deleting package source folder '" + deleteCandidate.getName() + "'"));
                }
    ...
    public List<RefactoringStatusEntry> performRemoteDelete(IProgressMonitor monitor)
        ...
        if (!ContainerDelegate.getInstance().getServiceLocator().getProjectService().isManagedOnlineProject(refactorModel.getProject())) {
            logger.warn("Unable to delete deploy for project '" + refactorModel.getProject().getName()
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                if (packageManifestCache != null) {
                    ...
                    for (Component deleteComponent : deleteComponents) {
                        ...
                        if (componentTypeNode != null && !componentTypeNode.hasChildNodes()) {
                            PackageManifestDocumentUtils.removeComponentTypeNode(packageManifestCache, deleteComponent
                                    .getComponentType());
                        }
                    }
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("Deleted component(s) from cache");
